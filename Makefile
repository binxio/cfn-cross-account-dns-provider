...
deploy-provider:  ## deploy the provider to the current account
	sed -i '' -e 's^$(NAME):[0-9]*\.[0-9]*\.[0-9]*[^\.]*^$(NAME):$(VERSION)^' cloudformation/cfn-cross-account-dns-provider.yaml
	aws cloudformation deploy \
                --stack-name $(NAME) \
                --capabilities CAPABILITY_IAM \
                --template-file ./cloudformation/cfn-cross-account-dns-provider.yaml
